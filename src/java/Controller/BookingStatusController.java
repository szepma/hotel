...
public class BookingStatusController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            //addNewStatus
            if (request.getParameter("task").equals("addNewBookingStatus")) {
                JSONObject returnValue = new JSONObject();
                
                if (!request.getParameter("status").isEmpty()) {
                    String status = request.getParameter("status");
                    
                    BookingStatus bs = new BookingStatus(0, status);
                    
                    String result = BookingStatusService.addNewBookingStatus(bs);
